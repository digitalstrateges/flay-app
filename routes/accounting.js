...
router.post('/ai/generate-bio', auth, async (req, res) => {
    try {
        ...
        const bio = await gemini.generateBio(name || 'Utilisateur', profession || 'Professionnel', city || 'Abidjan', style);
        res.json({ bio });
    } catch (e) {
...
router.post('/ai/generate-services', auth, async (req, res) => {
    try {
        ...
        const services = await gemini.generateServices(profession, context);
        res.json({ services });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/ai/analyze-image', auth, async (req, res) => {
    try {
        ...
        const analysis = await gemini.analyzeImage(imageBase64, question);
        res.json({ analysis });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/ai/marketing', auth, async (req, res) => {
    try {
        ...
        const content = await gemini.generateMarketingContent(businessName, profession, platform, offer);
        res.json({ content });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/ai/analyze-profile', auth, async (req, res) => {
    try {
        ...
        const user = db.get('users', req.userId);
        const analysis = await gemini.analyzeProfile({
            name: user?.name,
            title: profile?.title,
            bio: profile?.bio,
...
router.post('/ai/chat', auth, async (req, res) => {
    try {
        ...
        const user = db.get('users', req.userId);
        const profile = db.get('profiles', req.userId);
        const response = await gemini.chatResponse(message, {
