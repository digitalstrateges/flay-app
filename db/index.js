...
CREATE TABLE IF NOT EXISTS profiles (
    ...
    migrate() {
        const migrations = [
            "ALTER TABLE reservations ADD COLUMN type TEXT DEFAULT 'service'",
            "ALTER TABLE reservations ADD COLUMN duration TEXT DEFAULT '60'"
        ];
        for (const sql of migrations) {
            try { this.db.run(sql); } catch (e) { /* column may already exist */ }
        }
        try {
            const users = this.db.exec("SELECT id, password, salt FROM users WHERE salt != '' AND password NOT LIKE '{%'");
            if (users.length > 0) {
                const crypto = require('crypto');
                for (const row of users[0].values) {
                    const [id, oldPass, salt] = row;
                    if (oldPass && salt) {
                        const newObj = JSON.stringify({ salt, hash: oldPass, iterations: 10000 });
                        this.db.run("UPDATE users SET password = ?, salt = '' WHERE id = ?", [newObj, id]);
                    }
                }
            }
        } catch (e) { /* migration may fail if no old formats exist */ }
    }
    ...
    seed() {
        ...
        const demoPass = security.hashPassword('demo123');

        this.insert('users', {
            id: demoId, email: 'demo@flay.app', name: 'Jean Koffi', username: 'jean-koffi',
            password: JSON.stringify(demoPass), salt: '',
